...
# Dummy avatar if no filename provided
...
try:
    ...
    DB_CONN_STRING = os.getenv('DB_CONN_STRING', 'postgres://sketch_user:YourPwdShouldBeLongAndSecure!@' + rds_host_endpoint + '/sketchdb')
except Exception as e:
    raise e

def generate_legacy_path(n):
    randomlist = random.sample(range(0, 1000), n)
    for num in randomlist:
        yield f"image/avatar-{num}.png"

def generate_prod_path(n,rnd=0):
    randomlist = random.sample(range(0, 1000), n)
    for num in randomlist:
        yield f"avatar/avatar-{num}{rnd}.png"

def generate_aux_number(n):
    randomlist = random.sample(range(0, 1000), n)
    for num in randomlist:
        return num

def upload_schema(connection):
    try:
        cur = conn.cursor()
        cur.execute("select * from information_schema.tables where table_name=%s", ('avatars',))
        if bool(cur.rowcount) == False:
            cur.execute(open("schema.sql", "r").read())
    except Exception as e:
        logging.error(f"Error {e} while seeding the database")
        sys.exit(1)

def clear_table(connection):
    try:
        cur = conn.cursor()
        cur.execute("truncate avatars")
        cur.execute("delete from avatars")
        conn.commit()
    except Exception as e:
        logging.error(f"Error {e} while clearing the avatars table")
        sys.exit(1)
...
def clear_old_s3_objects(bucket):
    try:
        objects = []
        s3_conn = boto3.client('s3')
        obj_list = s3_conn.list_objects(Bucket=bucket)
        ...
        if len(objects) != 0:
            for rm in objects:
                s3_conn.Object(bucket, rm).delete()
    except Exception as e:
        logging.error(f"Error while cleaning s3 storage {bucket} {e}")
        sys.exit(1)
...
if __name__ == "__main__":
    ...
    try:
        ...
        logging.error(f"Failed to seed the db schema {e}")
        sys.exit(1)
    ...
    try:
        ...
        logging.error(f"Failed to clear the avatars table {e}")
        sys.exit(1)

    # Initialize s3 legacy resource
    try:
        s3_legacy = boto3.resource('s3',
                            endpoint_url=S3_LEGACY_ENDPOINT_URL,
                            aws_access_key_id=AWS_ACCESS_KEY_ID,
                            aws_secret_access_key=AWS_SECRET_ACCESS_KEY,
                            region_name=AWS_DEFAULT_REGION
                            )
    except Exception as e:
        logging.error(f"Error while connecting to legacy S3: {e}")
        sys.exit(1)

    # Initialize s3 production resource
    try:
        s3_production = boto3.resource('s3',
                            endpoint_url=S3_PRODUCTION_ENDPOINT_URL,
                            aws_access_key_id=AWS_ACCESS_KEY_ID,
                            aws_secret_access_key=AWS_SECRET_ACCESS_KEY,
                            region_name=AWS_DEFAULT_REGION
                            )
    except Exception as e:
        logging.error(f"Error while connecting to production S3: {e}")
        sys.exit(1)

    # Clear s3 storages if not empty
    try:
        for b in S3_LEGACY_BUCKET_NAME, S3_PRODUCTION_BUCKET_NAME:
            clear_old_s3_objects(b)
    except Exception as e:
        raise e

    # Generate as many legacy avatars as requested
    for path in generate_legacy_path(args.number_of_avatars):
        insert_db_row(conn, S3_LEGACY_ENDPOINT_URL + '/' + S3_LEGACY_BUCKET_NAME + '/' + path)
        create_s3_object(s3_legacy, S3_LEGACY_BUCKET_NAME, path)

    # Generate as many production avatars as requested
    for path in generate_prod_path(args.number_of_avatars, generate_aux_number(args.number_of_avatars)):
        insert_db_row(conn, S3_PRODUCTION_ENDPOINT_URL + '/' + S3_PRODUCTION_BUCKET_NAME + '/' + path)
        create_s3_object(s3_production, S3_PRODUCTION_BUCKET_NAME, path)

    print(f"Created {args.number_of_avatars} legacy and production avatars")
